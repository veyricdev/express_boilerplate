      ...
      <Table>
        <TableHeader className='bg-muted/50'>
          <TableRow className='hover:bg-transparent border-b'>
            ...
            <TableHead className='w-[30%] px-6 py-4 font-bold text-foreground h-12 uppercase text-[11px] tracking-wider'>Tên phòng ban</TableHead>
            <TableHead className='px-6 py-4 font-bold text-foreground h-12 uppercase text-[11px] tracking-wider'>Mô tả</TableHead>
            ...
            departments.map((dept) => (
              <TableRow key={dept.id} className='hover:bg-muted/30 transition-all border-b last:border-0 group'>
                ...
                <TableCell className='px-6 py-5 font-medium'>
                  <Badge variant='secondary' className='bg-primary/5 text-primary border-primary/10 font-bold px-3 py-1 rounded-lg group-hover:bg-primary group-hover:text-white transition-all duration-300'>
                    {dept.name}
                  </Badge>
                </TableCell>
                <TableCell className='px-6 py-5 text-muted-foreground text-sm max-w-[200px] truncate' title={dept.description || ''}>
                  {dept.description || '-'}
                </TableCell>
                <TableCell className='px-6 py-5'>
                  {dept.isActive ? (
                    ...
                    <Badge variant='destructive' className='px-3 py-1 rounded-full font-bold text-[10px] uppercase tracking-tighter'>
                      Tạm ngưng
                    </Badge>
                  )}
                </TableCell>
                    ...
                    {has(PERM_DEPARTMENTS_UPDATE) && (
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button size='icon' variant='ghost' className='h-9 w-9 rounded-full hover:bg-primary/10 hover:text-primary transition-all active:scale-90' onClick={() => onEdit(dept)}>
